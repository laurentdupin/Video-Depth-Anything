...
void main(){uint x=gl_GlobalInvocationID.x,y=gl_GlobalInvocationID.y;if(x>=p.width||y>=p.height)return;imageStore(output_image,ivec2(x,y),vec4(source_data.data[y*p.width+x]));}
